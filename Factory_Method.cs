...
class AnotherTransCom : TransportCompany
{
    public AnotherTransCom(string name)
        : base(name)
    { }
    public override TransportService Create(string n, int t)
    {
...
class Factory_Method
{
    ...
    static void Main()
    {
        ...
        TransportCompany aCom = new AnotherTransCom("Пьяный водитель");
        compService = aCom.Create("Пьяный водитель", 4);
        double dista = 100;
        Print(compService, dista);
    }
}