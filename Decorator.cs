...
class AutoClass : DecoratorOptions
{
    public AutoClass(AutoBase p, string t, string c, string s) : base(p, t, c, s)
    {
        AutoProperty = p;
...
class Decorator
{
    static void Main(string[] args)
    {
        ...
        AutoBase myreno = new MediaNAV(reno, "Навигация", "blue", "6");
        Print(myreno);

        AutoBase newmyReno = new SystemSecurity(new MediaNAV(reno, "Навигация", "blue", "6"),
            "Безопасность", "blue", "8");
        Print(newmyReno);

        AutoBase auto = new AutoClass(reno, "Навигация", "blue", "6");
        Print(auto);
    }
